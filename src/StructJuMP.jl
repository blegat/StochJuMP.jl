...
function StructuredModel(;solver=JuMP.UnsetSolver(), parent=nothing, same_children_than=nothing, num_scenarios::Int=0, prob::Float64=default_probability(parent))
    ...
    if same_children_than !== nothing
      probability = same_children_than.ext[:Stochastic].probability
      children = same_children_than.ext[:Stochastic].children
    else
      probability = Float64[]
      children = JuMP.Model[]
    end
